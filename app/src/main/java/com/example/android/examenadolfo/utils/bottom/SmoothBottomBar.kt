    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1
            ...
            for ((index, item) in items.withIndex()) {
                ...
                if (index== 2) {
                    a=  ( item.rect.centerX()
                        .toInt() + itemIconSize.toInt() / 2 - ((textLength / 2) * (1 - (OPAQUE - item.alpha) / OPAQUE.toFloat())).toInt())+12
                }

                else if (index== 4) {
                    a=  ( item.rect.centerX()
                        .toInt() + itemIconSize.toInt() / 2 - ((textLength / 2) * (1 - (OPAQUE - item.alpha) / OPAQUE.toFloat())).toInt())-3
                }
                else {
                    a=  ( item.rect.centerX()
                ...
                var a1: Int= 0
                if (index== 2) {
                    a1=  ( item.rect.centerX()
                        .toInt() - itemIconSize.toInt() / 2 - ((textLength / 2) * (1 - (OPAQUE - item.alpha) / OPAQUE.toFloat())).toInt())+1
                }
                else {
                    a1=  ( item.rect.centerX()
                        .toInt() - itemIconSize.toInt() / 2 - ((textLength / 2) * (1 - (OPAQUE - item.alpha) / OPAQUE.toFloat())).toInt())
